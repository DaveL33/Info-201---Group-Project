        ...
        sidebarLayout(# Group Members
          ...
          mainPanel(tabsetPanel(
            ...
            tabPanel(
              ...
              br(),
              p(
                "For this project, we will be learning to work with the Shiny framework for R. Since there are thousands of different items in Runescape, we will need an interactive web application that allows users to query information about specific in-game items, most likely via drop down inputs. The Shiny app will allow users to ask questions about the data set, and we will also use Plotly to visualize information that we find about our data set, via charts and graphs."
              ),
              br(),
              p(
                "The goal of this project is to use statistical analysis on the Runescape economy that would help players make in-game choices and help them earn more gold coins. We anticipate that working with Runescape's API will be difficult, as in our research we have found reports that their API is not the easiest to work with. We also expect learning to use Shiny will be a challenge, as it is a new framework for everyone involved in the project."
              ),
              br(), br(), br(), br(), br(), br(), br(), br()
            )
          )))
